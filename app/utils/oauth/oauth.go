...
func getAccessToken(accessTokenID string) (*accessToken, *errors.ResponseError) {
	...
	if err := json.Unmarshal(response.Body(), &atr); err != nil {
		return nil, errors.FormatError(http.StatusInternalServerError, "failed", "error when trying to unmarshal access token response")
	}

	var at accessToken
	at.ID = atr.Data.(map[string]interface{})["access_token"].(string)
	at.UserID = int64(atr.Data.(map[string]interface{})["user_id"].(float64))
	at.ClientID = int64(atr.Data.(map[string]interface{})["client_id"].(float64))
